...
class BrowserTtsService {
  ...
  async preload(texts: string[]): Promise<void> {
    ...
    const voice = getSelectedKokoroVoiceId();
    try {
      ...
      const workerCount = Math.max(1, this.kokoroWorkers.length);
      let workerCursor = 0;
      ...
      for (const content of normalized) {
        ...
        for (const chunk of chunks) {
          ...
          if (cached) continue;
          const worker = this.kokoroWorkers[workerCursor % workerCount];
          workerCursor += 1;
          if (!worker) continue;
      ...
      emitTtsDebug({
        level: "warn",
        stage: "preload",
        message: "TTS preload skipped due to warm cache failure.",
