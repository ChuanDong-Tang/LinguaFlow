...
export function MemoryRoundScreen({
  ...
  const [summary, setSummary] = useState({ remembered: 0, refreshed: 0, total: 0 });
      ...
      const resumed = stored
        ? {
            ...
            questions: stored.questions.flatMap((question) => {
              ...
              return [{ ...question, title: candidate.displayTitle, thumbnailUrl: candidate.thumbnail?.url ?? null }];
            }),
          }
        : null;
  ...
  const chooseOption = async (option: string) => {
    ...
    else updateQuestion((current) => ({ ...current, disabledOptions: [...current.disabledOptions, option] }));
    setTimeout(() => {
  ...
  const checkSentence = async () => {
    ...
    recordFirstAttempt(correct);
    if (correct) completeQuestion(question.firstAttemptCorrect ?? true);
    else setSentenceIncorrect(true);
    setTimeout(() => {
      if (!mountedRef.current) return;
      answerActionLocked.current = false;
      setChecking(false);
  ...
  const continueRound = async () => {
    ...
    if (nextIndex >= round.questions.length) {
      const remembered = round.questions.filter((item) => item.firstAttemptCorrect === true).length;
      const refreshed = round.questions.filter((item) => item.firstAttemptCorrect === false).length;
      setSummary({ remembered, refreshed, total: round.questions.length });
      setPhase("summary");
      setRound(null);
    ...
    Animated.timing(transition, { toValue: 1, duration: 240, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
  };
  ...
  return <SafeAreaView style={[styles.page, { backgroundColor: `${currentColor}20` }]}>
    ...
    <Progress total={round.questions.length} current={round.currentIndex} currentCompleted={question.completed} pulse={pulse} colors={["#8FD5C2", "#8CC8F0", "#F5BC91", "#B5A1E6"]} />
      ...
      <ScrollView contentContainerStyle={styles.questionScroll} showsVerticalScrollIndicator={false}>
        ...
        {question.kind === "choice" ? <>
          ...
          <View style={styles.options}>{question.options.map((option) => {
            ...
            return <Pressable key={option} disabled={disabled || question.completed || checking} style={[styles.option, disabled && styles.optionWrong, isAnswer && styles.optionCorrect]} onPress={() => void chooseOption(option)}><Text style={[styles.optionText, disabled && styles.optionWrongText, isAnswer && styles.optionCorrectText]}>{option}</Text>{disabled ? <Ionicons name="close" size={20} color="#D56E6E" /> : isAnswer ? <Ionicons name="checkmark" size={20} color="#43816E" /> : null}</Pressable>;
          })}</View>
        </> : <>
          <View style={[styles.sentenceTray, sentenceIncorrect && styles.sentenceTrayWrong]}>{selectedTokens.length ? <View style={styles.tokenWrap}>{selectedTokens.map((token) => <Pressable key={token.id} disabled={question.completed} style={styles.selectedToken} onPress={() => { setSentenceIncorrect(false); updateQuestion((current) => ({ ...current, selectedTokenIds: current.selectedTokenIds.filter((id) => id !== token.id) })); }}><Text style={styles.selectedTokenText}>{token.text.trim()}</Text></Pressable>)}</View> : <Text style={styles.trayHint}>{t("memory_round.tap_words")}</Text>}</View>
          <View style={styles.tokenWrap}>{availableTokens.map((token) => <Pressable key={token.id} disabled={question.completed} style={styles.token} onPress={() => { setSentenceIncorrect(false); updateQuestion((current) => ({ ...current, selectedTokenIds: [...current.selectedTokenIds, token.id] })); }}><Text style={styles.tokenText}>{token.text.trim()}</Text></Pressable>)}</View>
          {!question.completed ? <Pressable disabled={selectedTokens.length !== question.tokens.length || checking} style={[styles.checkButton, selectedTokens.length !== question.tokens.length && styles.buttonDisabled, sentenceIncorrect && styles.checkButtonWrong]} onPress={() => void checkSentence()}><Text style={styles.checkButtonText}>{sentenceIncorrect ? t("memory_round.try_again") : t("memory_round.check")}</Text></Pressable> : <Text style={styles.completedSentence}>{question.sentence}</Text>}
        </>}
        {question.completed ? <Animated.View style={[styles.successPanel, { opacity: success, transform: [{ scale: success.interpolate({ inputRange: [0, 1], outputRange: [0.96, 1] }) }] }]}><Text style={styles.successText}>{t("memory_round.remembered_it")}</Text><Pressable onPress={() => onOpenCard(question.recordId)}><Text style={styles.viewCard}>{t("memory_round.view_card")}</Text></Pressable><Pressable style={styles.primaryButton} onPress={() => void continueRound()}><Text style={styles.primaryButtonText}>{round.currentIndex === round.questions.length - 1 ? t("memory_round.see_summary") : t("common.continue")}</Text></Pressable></Animated.View> : null}
      </ScrollView>
    </Animated.View>
  </SafeAreaView>;
}
...
function Header({ onClose }: { onClose: () => void }) {
  return <View style={styles.header}><Pressable accessibilityRole="button" style={styles.headerButton} onPress={onClose}><Ionicons name="close" size={24} color={theme.colors.text} /></Pressable><Text style={styles.headerTitle}>{t("memory_round.title")}</Text><View style={styles.headerButton} /></View>;
}

function Progress({ total, current, currentCompleted, pulse, colors }: { total: number; current: number; currentCompleted: boolean; pulse: Animated.Value; colors: string[] }) {
  return <View style={styles.progress}>{Array.from({ length: total }, (_, index) => <React.Fragment key={index}>{index > 0 ? <View style={[styles.connector, index <= current && { backgroundColor: colors[(index - 1) % colors.length] }]} /> : null}<Animated.View style={[styles.progressNode, index < current && { backgroundColor: colors[index % colors.length] }, index === current && { backgroundColor: colors[index % colors.length] }, index === current && !currentCompleted && { transform: [{ scale: pulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.2] }) }], opacity: pulse.interpolate({ inputRange: [0, 1], outputRange: [0.75, 1] }) }]}>{index < current || index === current && currentCompleted ? <Ionicons name="checkmark" size={10} color="#fff" /> : null}</Animated.View></React.Fragment>)}</View>;
}
...
const styles = StyleSheet.create({
  ...
  questionPage: { flex: 1 },
  questionScroll: { paddingHorizontal: 22, paddingBottom: 36 },
  memoryImage: { width: "100%", height: 176, marginTop: 8, borderRadius: 24, backgroundColor: "#E8ECEB" },
  titlePrompt: { minHeight: 96, marginTop: 8, paddingHorizontal: 20, paddingVertical: 18, borderRadius: 24, backgroundColor: "rgba(255,255,255,0.82)", flexDirection: "row", alignItems: "center", gap: 14 },
  titleDot: { width: 13, height: 13, borderRadius: 7 },
  titlePromptText: { flex: 1, color: theme.colors.text, fontSize: 19, lineHeight: 27, fontWeight: "600" },
  questionHeading: { marginTop: 26, marginBottom: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  eyebrow: { color: "#66706B", fontSize: 14, fontWeight: "600" },
  audioButton: { width: 38, height: 38, borderRadius: 19, backgroundColor: "rgba(255,255,255,0.82)", alignItems: "center", justifyContent: "center" },
  ...
  sentence: { color: theme.colors.text, fontSize: 24, lineHeight: 38, fontWeight: "500" },
  blank: { color: "#496F62", backgroundColor: "#DFF1E7", textDecorationLine: "underline", textDecorationColor: "#729B8B" },
  options: { marginTop: 26, gap: 12 },
  option: { minHeight: 58, paddingHorizontal: 18, borderRadius: 18, borderWidth: 1.5, borderColor: "rgba(93,113,104,0.18)", backgroundColor: "rgba(255,255,255,0.9)", flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  optionText: { color: theme.colors.text, fontSize: 17, fontWeight: "500" },
  ...
  tokenText: { color: theme.colors.text, fontSize: 16, fontWeight: "500" },
  ...
  selectedTokenText: { color: "#365E52", fontSize: 16, fontWeight: "500" },
  ...
  completedSentence: { marginTop: 18, color: "#397461", fontSize: 19, lineHeight: 29, fontWeight: "500" },
  successPanel: { marginTop: 26 },
  successText: { color: "#397461", fontSize: 16, fontWeight: "600", textAlign: "center" },
  viewCard: { marginTop: 16, color: theme.colors.textSecondary, fontSize: 14, textDecorationLine: "underline", textAlign: "center" },
  ...
  summaryLine: { color: theme.colors.textSecondary, fontSize: 18, lineHeight: 31, textAlign: "center" },
});
