...
export class SystemEventLogCleanupWorker {
  ...
  private async deleteInBatches(input: { createdBefore: Date; batchSize: number }): Promise<number> {
    ...
    // 轮次快照上界：限制在本轮开始前的数据，避免新增日志导致分页反复扫描
    const createdAtUpperBound = new Date();
    while (true) {
      const rows: Array<{ id: string }> = await this.prisma.systemEventLog.findMany({
        where: {
          createdAt: { lt: input.createdBefore, lte: createdAtUpperBound },
          ...(lastId
            ? {
                id: {
                  gt: lastId,
  ...
  private async writeRoundLog(input: {
    ...
    await this.writeWorkerLog({
      module: "infra",
      event: "infra.worker.system_event_log_cleanup_round",
      level: input.status === "failed" ? "error" : "info",
      status: input.status === "failed" ? "failed" : "success",
      metadata: {
        worker: "system_event_log_cleanup",
        status: input.status,
        ...
        lockKey: SystemEventLogCleanupWorker.LOCK_KEY,
  ...
  private async flushLockMissIfNeeded(force: boolean): Promise<void> {
    ...
    const now = Date.now();
    if (!force && now - this.lastLockMissReportAt < SystemEventLogCleanupWorker.LOCK_MISS_REPORT_WINDOW_MS) {
      return;
    }
    this.lastLockMissReportAt = now;
    await this.writeWorkerLog({
      module: "infra",
      event: "infra.worker.system_event_log_cleanup_lock_miss",
      level: "warn",
      status: "ignored",
      metadata: {
        worker: "system_event_log_cleanup",
        ...
        skipReason: "lock_not_acquired",
        lockMissCount: this.lockMissCount,
        reportWindowMs: SystemEventLogCleanupWorker.LOCK_MISS_REPORT_WINDOW_MS,
      },
    });
