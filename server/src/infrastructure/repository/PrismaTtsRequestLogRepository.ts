  ...
  async create(input: CreateTtsRequestLogInput): Promise<void> {
    await this.prisma.ttsRequestLog.create({
      data: {
        ...
        preparationMs: input.preparationMs ?? null,
        cacheLookupMs: input.cacheLookupMs ?? null,
        lockWaitMs: input.lockWaitMs ?? null,
        queueWaitMs: input.queueWaitMs ?? null,
        synthesisMs: input.synthesisMs ?? null,
        storageMs: input.storageMs ?? null,
        persistenceMs: input.persistenceMs ?? null,
